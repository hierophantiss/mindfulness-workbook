...
function showClassicalModal() {
  var existing = document.getElementById('binauralModal');
  if (existing) existing.remove();

  var isEl  = (typeof LANG !== 'undefined') && LANG === 'el';
  ...
  modal.innerHTML =
    ...
    + (isEl ? 'Κλασική Μουσική + Δέλτα (0.5–4 Hz)' : 'Classical Music + Delta (0.5–4 Hz)')
    + '</h2>'
    + '<p style="font-size:14px;color:#9aa4aa;line-height:1.7;margin-bottom:16px">'
    + (isEl
      ? 'FM σύνθεση πιάνου, εγχόρδων και τσέλο με Delta binaural 2 Hz για βαθύ ύπνο. Amplitude modulation και isochronic pulses λειτουργούν και χωρίς ακουστικά.'
      : 'FM synthesised piano, strings and cello with Delta binaural 2 Hz for deep sleep. Amplitude modulation and isochronic pulses work even without headphones.')
    + '</p>'
    ...
    + '<p style="font-size:13px;color:#c8922a;line-height:1.6;margin-bottom:8px;font-weight:600">⚠️ ' + (isEl ? 'Σημαντική ενημέρωση:' : 'Important notice:') + '</p>'
    + '<ul style="font-size:12px;color:#9a938c;line-height:1.7;padding-left:16px;margin:0">'
    + '<li>' + (isEl ? '<strong style="color:#c8922a">Ακουστικά</strong> για πλήρη εφέ binaural' : '<strong style="color:#c8922a">Headphones</strong> for full binaural effect') + '</li>'
    + '<li>' + (isEl ? '<strong style="color:#c8922a">Επιληψία</strong> ή σπασμοί: συμβουλευτείτε γιατρό πρώτα' : '<strong style="color:#c8922a">Epilepsy</strong> or seizures: consult a doctor first') + '</li>'
    + '<li>' + (isEl ? 'Σε <strong style="color:#c8922a">ζάλη</strong>, σταματήστε αμέσως' : 'If <strong style="color:#c8922a">dizzy</strong>, stop immediately') + '</li>'
    + '<li>' + (isEl ? 'Ένταση σε <strong style="color:#c8922a">χαμηλό επίπεδο</strong>' : 'Keep volume <strong style="color:#c8922a">low</strong>') + '</li>'
    + '</ul></div>'
    + '<div style="display:flex;gap:10px;flex-direction:column">'
    + '<button id="classicalAccept" style="background:#2a6b6e;color:white;border:none;padding:14px;border-radius:10px;font-family:inherit;font-size:14px;font-weight:600;cursor:pointer">🎹 '
    + (isEl ? 'Κατάλαβα — Ξεκίνα' : 'I understand — Start')
    + '</button>'
    + '<button id="classicalCancel" style="background:transparent;color:#6a7580;border:1px solid #1e2a34;padding:12px;border-radius:10px;font-family:inherit;font-size:13px;cursor:pointer">'
    + (isEl ? 'Ακύρωση' : 'Cancel')
    + '</button>'
    ...
    + '<input type="checkbox" id="classicalRemember" style="accent-color:#2a6b6e">'
    + (isEl ? 'Να μην εμφανιστεί ξανά' : "Don't show again")
    + '</label></div></div>';

  document.body.appendChild(modal);

  document.getElementById('classicalAccept').addEventListener('click', function() {
    if (document.getElementById('classicalRemember').checked) {
      localStorage.setItem('binaural_ack_delta', '1');
    }
    modal.style.display = 'none';
    startClassicalAudio();
  });
  document.getElementById('classicalCancel').addEventListener('click', function() {
    modal.style.display = 'none';
  });
