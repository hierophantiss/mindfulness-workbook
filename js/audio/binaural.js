...
function buildBreathAudio() {
  ...
  // Theta (4-8Hz) -> Relaxation, Mindfulness, SOS
  // Alpha (8-13Hz) -> Focus, Sharp Presence
  // Delta (0.5-4Hz) -> Deep Sleep
  var beatConfigs = {
    '4-2-6-1': { base: 180, beat: 6, pulse: 6 },  // Grounding -> Theta 6Hz
    '4-7-8': { base: 140, beat: 5, pulse: 5 },    // Calm -> Theta 5Hz
    '5-5': { base: 180, beat: 10, pulse: 10 },    // Focus -> Alpha 10Hz
    'sleep-delta': { base: 110, beat: 2.5, pulse: 2.5 }, // Sleep -> Delta 2.5Hz
    'sleep-classical': { base: 110, beat: 2.5, pulse: 2.5 } 
  };
  var config = beatConfigs[bCurrentPattern] || { base: 160, beat: 10, pulse: 10 };
  ...
  return { master: master, oceanG: oceanG, panner: panner };
}
...
function stopBreathAudio() {
  ...
  setTimeout(function() {
    breathAudioOn = false;
    try { breathAudioNodes.master.disconnect(); } catch(e) {}
...
function showBinauralModal() {
  ...
  modal.innerHTML = '<div style="max-width:380px;width:100%;background:#111820;border:1px solid #1e2a34;border-radius:16px;padding:28px 24px;text-align:center;font-family:DM Sans,sans-serif">'
    ...
    + '<h2 style="font-family:Cormorant Garamond,serif;font-size:22px;color:#5cbcbf;margin-bottom:12px;font-weight:600">' + beatType + ' Binaural Beats</h2>'
  ...
  document.getElementById('binauralAccept').addEventListener('click', function() { modal.style.display = 'none'; startBreathAudio(); });
...
showScreen = function(id) {
  if (breathAudioOn && id !== 'breath') stopBreathAudio();
  _origShowScreen(id);
};
