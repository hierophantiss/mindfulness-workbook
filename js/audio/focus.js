...
function toggleFocus() {
  ...
  if (!focusOn) {
    ...
    focusOn = false;
    if (focusNodes) focusNodes.master.gain.linearRampToValueAtTime(0, ac.currentTime + 2);
...
function stopBowlsOnly() {
  ...
  bowlsOn = false;
  var ac = getAC();
  if (bowlNodes) bowlNodes.master.gain.linearRampToValueAtTime(0, ac.currentTime + 1);
...
function stopFocusOnly() {
  ...
  focusOn = false;
  var ac = getAC();
  if (focusNodes) focusNodes.master.gain.linearRampToValueAtTime(0, ac.currentTime + 1);
  var btn = document.getElementById('focusToggle');
  var icon = document.getElementById('focusIcon');
  if (btn) btn.classList.remove('active');
  if (icon) icon.textContent = '♪';
