...
function bDraw() {
  ...
  const bAudioActive = (typeof window.breathAudioOn !== 'undefined' && window.breathAudioOn);
  const isDeepFocus = bSessionActive; 
  const cx = w / 2, groundY = h * (isDeepFocus ? 0.60 : 0.58); // Adjusted slightly higher
  
  // Dynamic scale based on mode 
  const baseW = isDeepFocus ? 220 : 210; // Increased scale for a stronger hero presence
