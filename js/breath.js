...
function bUpdateMainControls() {
  ...
  if (mainAudioBtn) {
    const textEl = mainAudioBtn.querySelector('.btn-text');
    if (textEl) {
      const activeText = (LANG === 'el' ? 'ΑΚΟΥΣΤΙΚΑ: ON' : 'AUDIO: ON');
      const inactiveText = (LANG === 'el' ? 'ΑΚΟΥΣΤΙΚΑ: OFF' : 'AUDIO: OFF');
...
function bDraw() {
  ...
  const bAudioActive = (typeof window.breathAudioOn !== 'undefined' && window.breathAudioOn);
  const isDeepFocus = bSessionActive; 
  const cx = w / 2, groundY = h * (isDeepFocus ? 0.65 : 0.62); // Lowered more to ensure head visibility
  
  // Dynamic scale based on mode 
  const baseW = isDeepFocus ? 160 : 220;
  const baseH = isDeepFocus ? 240 : 300;
