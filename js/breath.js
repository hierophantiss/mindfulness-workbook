...
function bUpdateMainControls() {
  ...
  if (mainAudioBtn) {
    const textEl = mainAudioBtn.querySelector('.btn-text');
    if (textEl) {
      textEl.textContent = bAudioActive ? t('btnAudioOn') : t('btnAudioOff');
    }
  ...
  if (playBtn) {
    ...
    if (ctrlCard) {
      // Immersive mode only if RUNNING
      const isDeepFocus = bRunning; 
      if (isDeepFocus) {
...
function bDraw() {
  ...
  const bAudioActive = (typeof window.breathAudioOn !== 'undefined' && window.breathAudioOn);
  const isDeepFocus = bRunning; // Only use immersive metrics if RUNNING
  const cx = w / 2, groundY = h * (isDeepFocus ? 0.48 : 0.62); // Lift figure EVEN HIGHER during focus
  
  // Dynamic scale based on mode — larger when active
  // 🔽 ΤΡΟΠΟΠΟΙΗΣΗ: μικραίνουμε τη φιγούρα
  const baseW = isDeepFocus ? 160 : 270;
  const baseH = isDeepFocus ? 220 : 360;
  ...
  if (bRunning || bAudioActive) {
    ...
    for (let i = 0; i < 3; i++) {
        const ringProg = (bTime * 0.4 + i * 0.33) % 1;
...
window.handleExitClick = function() {
  ...
  if (typeof stopBreathAudio === 'function') {
    stopBreathAudio();
    // Also reset common state flags if any
    window.breathAudioOn = false;
  }
  // Immediately return visuals to standard mode
  document.getElementById('screen-breath').classList.remove('deep-focus-active');
  const ctrlCard = document.getElementById('breathControls');
  if (ctrlCard) {
    ctrlCard.classList.remove('collapsed', 'deep-focus-mode');
  }
  bUpdateMainControls();
};
