...
function bUpdateMainControls() {
  ...
  if (playBtn) {
    ...
    if (ctrlCard) {
      const isDeepFocus = bRunning || bAudioActive;
      if (isDeepFocus) {
...
function bDraw() {
  ...
  const pulse = Math.sin(bTime * 1.5) * 0.5 + 0.5; // Slightly faster pulse for responsiveness
  ...
  // ── Shared Silence indicator (Simulated) ──
  const sharedCount = 42 + Math.floor(Math.sin(bTime * 0.05) * 8);
  bCtx.fillStyle = 'rgba(255,255,255,0.4)';
  bCtx.font = '500 10px Inter, sans-serif';
  bCtx.textAlign = 'center';
  bCtx.fillText(`${sharedCount} ${LANG === 'el' ? 'άτομα αναπνέουν μαζί σου' : 'people breathing with you'}`, 0, 78 + 15);
...
function initBreathExercise() {
  ...
  tryResize();
  if (!bInited) {
    document.getElementById('bPlayBtn').addEventListener('click', () => {
      const bAudioActive = (typeof window.breathAudioOn !== 'undefined' && window.breathAudioOn);
      if (bRunning) {
        bPause();
      } else {
