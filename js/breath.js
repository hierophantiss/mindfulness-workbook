...
const B_PATTERNS = {
  '4-2-6-1': {
    ...
    desc_el: 'Binaural Theta (6Hz) & Αργή εκπνοή — Gerritsen & Band (2018)',
    desc_en: 'Binaural Theta (6Hz) & Slow exhale — Gerritsen & Band (2018)',
    phases: [
  ...
  '4-7-8': {
    ...
    desc_el: 'Binaural Theta (5Hz) & Μέθοδος Dr. Andrew Weil',
    desc_en: 'Binaural Theta (5Hz) & Dr. Andrew Weil method',
    phases: [
      { dur: 4000, armFrom: 0, armTo: 1, labelKey: 0 },
  ...
  '5-5': {
    ...
    desc_el: 'Binaural Alpha (10Hz) & Καρδιακή συνοχή — McCraty et al. (2009)',
    desc_en: 'Binaural Alpha (10Hz) & Cardiac coherence — McCraty et al. (2009)',
    phases: [
  ...
  'sleep-delta': {
    ...
    desc_el: 'Binaural Δέλτα συχνότητα (2.5Hz) & Ήχοι Φύσης',
    desc_en: 'Binaural Delta frequency (2.5Hz) & Nature Sounds',
    phases: [
...
let bSessionActive = false; // New flag for immersive mode state
...
function updateBreathLang() {
  ...
  // Update main audio button label
  const mainAudioBtn = document.getElementById('bAudioMainBtn');
  if (mainAudioBtn) {
    const textEl = mainAudioBtn.querySelector('.btn-text');
    const bAudioActive = (typeof window.breathAudioOn !== 'undefined' && window.breathAudioOn);
    if (textEl) {
      textEl.textContent = bAudioActive ? (LANG === 'el' ? 'Ήχος On' : 'Audio On') : (LANG === 'el' ? 'Ήχος Off' : 'Audio Off');
    }
  }
...
function bUpdateMainControls() {
  ...
  const ctrlCard = document.getElementById('breathUiCard');
  const bAudioActive = (typeof window.breathAudioOn !== 'undefined' && window.breathAudioOn);

  // Sync main audio button if it exists
  const mainAudioBtn = document.getElementById('bAudioMainBtn');
  if (mainAudioBtn) {
    const textEl = mainAudioBtn.querySelector('.btn-text');
    if (textEl) {
      const activeText = (LANG === 'el' ? 'ΗΧΟΣ' : 'AUDIO');
      const inactiveText = (LANG === 'el' ? 'ΗΧΟΣ' : 'AUDIO');
      textEl.textContent = bAudioActive ? activeText : inactiveText;
    }
    mainAudioBtn.classList.toggle('active', bAudioActive);
  }
  ...
  if (playBtn) {
    ...
    if (iconEl && textEl) {
      playBtn.classList.toggle('running', bRunning);
      if (bRunning) {
        iconEl.textContent = '⏸';
        textEl.textContent = (LANG === 'el' ? 'ΣΤΟΠ' : 'STOP');
      } else {
        iconEl.textContent = '▶';
        textEl.textContent = ui.play;
      }
    }
    // Update card class for collapse/expand and deep focus
    if (ctrlCard) {
      // Immersive mode if session is active
      const isDeepFocus = bSessionActive; 
      if (isDeepFocus) {
        ctrlCard.classList.add('collapsed');
        ctrlCard.classList.add('deep-focus-mode');
        document.getElementById('screen-breath').classList.add('deep-focus-active');
      } else {
        ctrlCard.classList.remove('collapsed');
        ctrlCard.classList.remove('deep-focus-mode');
        document.getElementById('screen-breath').classList.remove('deep-focus-active');
      }
    }
  }
...
function bResizeCanvas() {
  ...
  bCanvas.width = Math.round(rect.width * dpr);
  bCanvas.height = Math.round(rect.height * dpr);
...
// ── Pattern Colors (Research-based: Green for Grounding, Blue for Calm, Gold for Coherence, Purple for Deep Sleep) ──
const B_COLORS = {
  '4-2-6-1': { primary: '#3A5E57', secondary: '#60857D', aura: '70, 130, 120' }, // Teal Earth
  '4-7-8':   { primary: '#2A4D69', secondary: '#4B86B4', aura: '100, 150, 190' }, // Deep Water Calm
  '5-5':     { primary: '#C5A059', secondary: '#E8C547', aura: '220, 190, 90' },  // Golden Coherence
  'sleep-delta': { primary: '#4B2C5E', secondary: '#7F5283', aura: '120, 80, 150' }, // Delta Purple
  'sleep-classical': { primary: '#8B5E3C', secondary: '#B98B53', aura: '185, 140, 85' } // Warm Classical Amber
};
...
function bDraw() {
  ...
  const baseW = isDeepFocus ? 220 : 210; 
  const baseH = isDeepFocus ? 330 : 300; 
  const sc = Math.min(w / baseW, h / baseH);
  
  const cx = w / 2;
  // Center vertically dynamically. The drawing spans from roughly y = -162 (head) to 44 (globe bottom).
  // Center point locally is y = -59. So we offset groundY by +59.
  const groundY = (h / 2) + (59 * sc);
  
  const pulse = Math.sin(bTime * 1.5) * 0.5 + 0.5;
  const breath = Math.sin(bTime * 0.9) * 0.5 + 0.5;
  const pCol = B_COLORS[bCurrentPattern] || B_COLORS['4-2-6-1'];

  // ── Atmospheric background (behind figure, untransformed) ──
  const auraIntense = (bRunning || bAudioActive) ? (0.15 + (bRunning ? bArmPos : pulse) * 0.15 + pulse * (bAudioActive ? 0.12 : 0.05)) : 0.08;
  const bgGrad = bCtx.createRadialGradient(cx, groundY - h*0.2, 0, cx, groundY - h*0.2, Math.max(w,h)*0.8);
  ...
  // ── Show static but pulsing hero if audio is on but breathing logic is NOT running ──
  let visualArmPos = bArmPos;
  if (!bRunning && bAudioActive) {
      visualArmPos = pulse * 0.3; // Gentle automatic pulse
  }

  // Spreading Rainbow Aura Effect (spreading across the screen)
  if (bRunning || bAudioActive) {
    bCtx.save();
    bCtx.globalCompositeOperation = 'screen';
    for (let i = 0; i < 3; i++) {
        const ringProg = (bTime * 0.25 + i * 0.33) % 1; // Slower for focus
        const ringSc = ringProg * 1.5 * Math.max(w, h);
        const ringAlpha = Math.sin(ringProg * Math.PI) * (bAudioActive ? 0.15 : 0.12) * (visualArmPos > 0 ? Math.max(0.2, visualArmPos) : (bRunning ? 0.2 : 0.4));
        
        let ringColor = `rgba(${pCol.aura}, ${ringAlpha})`;
        if (bAudioActive) {
           const hue = (performance.now() / 50 + i * 40) % 360;
           ringColor = `hsla(${hue}, 60%, 75%, ${ringAlpha})`;
        }

        const ringGrad = bCtx.createRadialGradient(cx, groundY - 60*sc, ringSc * 0.8, cx, groundY - 60*sc, ringSc);
        ringGrad.addColorStop(0, 'rgba(255,255,255,0)');
        ringGrad.addColorStop(0.5, ringColor);
        ringGrad.addColorStop(1, 'rgba(255,255,255,0)');
        bCtx.fillStyle = ringGrad;
        bCtx.beginPath();
        bCtx.arc(cx, groundY - 60*sc, ringSc, 0, Math.PI*2);
        bCtx.fill();
    }
    bCtx.restore();
  }
  ...
  // thin horizon stroke
  bCtx.strokeStyle = `rgba(${pCol.aura}, 0.10)`;
  bCtx.lineWidth = 1;
  bCtx.beginPath();
  ...
  bCtx.fillStyle = shG;
  bCtx.beginPath(); bCtx.ellipse(0, 45, 60 + pulse*5, 10 + pulse*2, 0, 0, Math.PI*2); bCtx.fill();
  ...
  const lightAura = (bRunning || bAudioActive) ? (0.25 + visualArmPos * 0.15 + pulse * (bAudioActive ? 0.15 : 0.05)) : 0.25;
  ...
  if (bRunning) {
    ...
    bCtx.strokeStyle = `rgba(${pCol.aura}, 0.18)`;
    bCtx.lineWidth = 8;
    bCtx.beginPath(); bCtx.arc(0, gY, gR+7, aS, aE); bCtx.stroke();
    // crisp arc
    bCtx.strokeStyle = `rgba(${pCol.aura}, 0.8)`;
    bCtx.lineWidth = 2.4;
    bCtx.beginPath(); bCtx.arc(0, gY, gR+7, aS, aE); bCtx.stroke();
    // leading dot
    bCtx.shadowBlur = 10;
    bCtx.shadowColor = `rgb(${pCol.aura})`;
  ...
  if (visualArmPos > 0.05) {
    bCtx.fillStyle = `rgba(255,240,210,${visualArmPos*0.06})`;
    bCtx.beginPath();
    bCtx.ellipse(0, cY+2, 14 + visualArmPos*4, 8 + visualArmPos*3, 0, 0, Math.PI*2);
    bCtx.fill();
  }
  // collar
  ...
  for (let side = -1; side <= 1; side += 2) {
    ...
    // Smooth transition to lap when visualArmPos is near 0
    const holdWeight = Math.max(0, 1 - (visualArmPos / 0.25));
    
    // Mudra target coordinates
    const targetEx = side * 16;
    const targetEy = sy + 22;
    const targetHx = side * 4; 
    const targetHy = mY;
    ...
    bCtx.closePath(); bCtx.fill();
    // sleeve highlight
    bCtx.strokeStyle = `rgba(180,200,185,${0.12 + visualArmPos*0.08})`;
    bCtx.lineWidth = 0.9;
    bCtx.beginPath();
    ...
    bCtx.stroke();
    // elbow joint
    bCtx.fillStyle = `rgba(40,60,52,${0.5 + visualArmPos*0.15})`;
    ...
    // hand
    bCtx.fillStyle = `rgba(215,185,135,${0.55 + visualArmPos*0.22})`;
    bCtx.beginPath(); bCtx.arc(hx, hy, 5, 0, Math.PI*2); bCtx.fill();
    // hand rim
    bCtx.strokeStyle = `rgba(140,105,65,${0.3 + visualArmPos*0.15})`;
    bCtx.lineWidth = 0.7;
    bCtx.beginPath(); bCtx.arc(hx, hy, 5, 0, Math.PI*2); bCtx.stroke();
    // energy trail along arm (during active phases)
    if (visualArmPos > 0.1 && (bRunning || bAudioActive)) {
      bCtx.strokeStyle = `rgba(${pCol.aura},${visualArmPos*0.10})`;
      bCtx.lineWidth = 8;
      bCtx.beginPath();
      bCtx.moveTo(sx, sy);
      bCtx.quadraticCurveTo(eX, eY, hx, hy);
      bCtx.stroke();
      bCtx.strokeStyle = `rgba(220,240,250,${visualArmPos*0.18})`;
      bCtx.lineWidth = 1.2;
      bCtx.beginPath();
      bCtx.moveTo(sx, sy);
      bCtx.quadraticCurveTo(eX, eY, hx, hy);
  ...
  if (mudraWeight > 0) {
    bCtx.save();
    bCtx.globalAlpha = mudraWeight;
    const mYLocal = wY+16; // Use a distinct variable just in case
    // palm gradient
    const mG = bCtx.createRadialGradient(0, mYLocal-1, 1, 0, mYLocal, 10);
    mG.addColorStop(0, 'rgba(220,190,140,0.55)');
    mG.addColorStop(1, 'rgba(180,150,100,0.3)');
    bCtx.fillStyle = mG;
    bCtx.beginPath(); bCtx.ellipse(0, mYLocal, 10, 6, 0, 0, Math.PI*2); bCtx.fill();
    // thumb line
    bCtx.strokeStyle = 'rgba(150,115,75,0.35)'; bCtx.lineWidth = 1;
    bCtx.beginPath(); bCtx.arc(0, mYLocal, 6, 0.15, Math.PI-0.15); bCtx.stroke();
    // soft glow on mudra during pause
    bCtx.fillStyle = `rgba(255,220,150,${0.08 + pulse*0.05})`;
    bCtx.beginPath(); bCtx.ellipse(0, mYLocal, 16, 8, 0, 0, Math.PI*2); bCtx.fill();
    bCtx.restore();
  }

  // ── INFINITY SYMBOL (The Pulsing Heart) ──
  const iY = cY+9, iR = 9, iW = 16;
  const iPulse = 0.5 + 0.5 * Math.sin(bTime * 3); // Faster, energetic blink
  
  // Infinity Glow
  const iGlow = bCtx.createRadialGradient(0, iY, 0, 0, iY, iR * 2);
  iGlow.addColorStop(0, `rgba(${pCol.aura}, ${0.4 * iPulse})`);
  iGlow.addColorStop(1, 'rgba(255,255,255,0)');
  bCtx.fillStyle = iGlow;
  bCtx.beginPath(); bCtx.ellipse(0, iY, iW, iR, 0, 0, Math.PI*2); bCtx.fill();

  // Infinity Rainbow Path
  bCtx.save();
  bCtx.translate(0, iY);
  bCtx.lineWidth = 1.6;
  ...
  for (let i = 0; i < colors.length; i++) {
    ...
    bCtx.globalAlpha = (0.3 + 0.7 * pulse) * (1 - Math.abs(i-3)/7);
    bCtx.beginPath();
    bCtx.moveTo(0, offset);
    bCtx.bezierCurveTo(-iR*1.3, -iR+offset, -iR*1.3, iR+offset, 0, offset);
    bCtx.bezierCurveTo(iR*1.3, -iR+offset, iR*1.3, iR+offset, 0, offset);
    bCtx.stroke();
  }
  bCtx.restore();
  ...
  if (bRunning || bAudioActive) {
    ...
    // thin halo ring
    bCtx.strokeStyle = `rgba(${pCol.aura}, ${0.18 + pulse*0.08})`;
    bCtx.lineWidth = 0.8;
  ...
  if ((bRunning || bAudioActive) && visualArmPos > 0.15) {
    const aurColors = [
      [168,213,220], // inhale
      [210,210,210], // hold
      [230,200,120], // exhale
    ...
    const col = bRunning ? aurColors[bPhaseIdx] : [168,213,220]; // Default to teal
    for (let i = 0; i < 4; i++) {
  ...
  // Position the HTML overlay perfectly centered inside the globe
  const overlay = document.getElementById('breathPhaseOverlay');
  if (overlay) {
    // Globe is drawn at y=0 relative to groundY
    overlay.style.top = groundY + 'px';
    // Optionally scale it slightly if needed, but centering should be enough
  }
...
function bAnimate(ts) {
  ...
  if (bCanvas.width !== targetW || bCanvas.height !== targetH) {
    bResizeCanvas();
  } else if (!bCanvas.width) {
    bResizeCanvas();
  }
...
function bPause() {
  if (!bRunning) return;
  bRunning = false;
  // Binaural stays active as requested
  bUpdateMainControls();
...
function switchPattern(key) {
  ...
  tapFeedback();
  var wasAudio = window.breathAudioOn;
  bCurrentPattern = key;
  ...
  // If audio was ON, restart it to load the new pattern's HQ nature layer
  if (wasAudio && typeof stopBreathAudio === 'function' && typeof startBreathAudio === 'function') {
    // We force stop and restart
    // Note: stopBreathAudio has a fade, so we wait slightly or just overwrite
    stopBreathAudio();
    setTimeout(() => {
        if (!window.breathAudioOn) startBreathAudio();
    }, 500);
  }
}
...
// Global handler for exit/back to card
window.handleExitClick = function() {
  bPause();
  bSessionActive = false;
  if (typeof stopBreathAudio === 'function') {
    stopBreathAudio();
...
// Global handler for play/pause to ensure reliability
window.handlePlayClick = function() {
  const bAudioActive = (typeof window.breathAudioOn !== 'undefined' && window.breathAudioOn);
  if (bRunning) {
    bPause();
  } else {
    // If binaural is NOT on, check if there's a remembered choice
    if (!bAudioActive) {
      const choice = localStorage.getItem('binaural_init_choice');
      if (choice === 'yes') {
...
function initBreathExercise() {
  ...
  // Create ResizeObserver to handle immersive layout changes
  if (window.ResizeObserver) {
    const observer = new ResizeObserver(() => {
      bResizeCanvas();
      bDraw();
    });
    observer.observe(document.querySelector('.breath-hero-area'));
  }
...
function showBinauralPrompt() {
  const existing = document.getElementById('binaural-prompt');
  if (existing) existing.remove();

  const isEl = LANG === 'el';
  const overlay = document.createElement('div');
  overlay.id = 'binaural-prompt';
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.8);display:flex;align-items:center;justify-content:center;z-index:1000;animation:fadeIn 0.2s ease-out;padding:20px';
  overlay.innerHTML = `
    <div style="background:var(--sidebar-bg);padding:24px;border-radius:16px;max-width:320px;width:100%;text-align:center;border:1px solid var(--border);box-shadow:0 10px 40px rgba(0,0,0,0.4)">
      <div style="font-size:32px;margin-bottom:12px">🎧</div>
      <h3 style="margin-bottom:8px;font-family:inherit;color:var(--text)">${t('binauralPromptTitle')}</h3>
      <p style="font-size:13px;color:var(--text-soft);margin-bottom:20px;line-height:1.5">${t('binauralPromptSub')}</p>
      
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:20px;justify-content:center;cursor:pointer" id="bpRememberWrapper">
        <input type="checkbox" id="bpRemember" style="accent-color:var(--teal);width:16px;height:16px;cursor:pointer">
        <label for="bpRemember" style="font-size:13px;color:var(--text-soft);cursor:pointer">${isEl ? 'Να μην ερωτηθώ ξανά' : "Don't ask me again"}</label>
      </div>

      <div style="display:flex;flex-direction:column;gap:8px">
        <button id="bp-yes" class="btn-primary" style="width:100%;justify-content:center">${t('binauralPromptYes')}</button>
        <button id="bp-no" class="btn-secondary" style="width:100%;justify-content:center;background:none">${t('binauralPromptNo')}</button>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);

  document.getElementById('bpRememberWrapper').onclick = function(e) {
    if (e.target.id !== 'bpRemember') {
      var cb = document.getElementById('bpRemember');
      cb.checked = !cb.checked;
    }
  };

  document.getElementById('bp-yes').onclick = () => {
    if (document.getElementById('bpRemember').checked) {
      localStorage.setItem('binaural_init_choice', 'yes');
    }
    overlay.remove();
    if (typeof toggleBreathAudio === 'function') toggleBreathAudio();
    bStart();
  };
  document.getElementById('bp-no').onclick = () => {
    if (document.getElementById('bpRemember').checked) {
      localStorage.setItem('binaural_init_choice', 'no');
    }
    overlay.remove();
    bStart();
  };
...
window.initBreathExercise = initBreathExercise;
window.bStart = bStart;
window.bPause = bPause;
window.bReset = bReset;
window.switchPattern = switchPattern;
window.toggleBreathAudio = typeof toggleBreathAudio !== 'undefined' ? toggleBreathAudio : undefined;
window.toggleBreathVoice = toggleBreathVoice;
window.B_PATTERNS = B_PATTERNS;
window.bRunning = typeof bRunning !== 'undefined' ? bRunning : false;
window.bCurrentPattern = typeof bCurrentPattern !== 'undefined' ? bCurrentPattern : '4-2-6-1';
window.B_PHASES = typeof B_PHASES !== 'undefined' ? B_PHASES : undefined;
window.bUpdatePhase = bUpdatePhase;
