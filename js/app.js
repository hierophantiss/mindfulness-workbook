...
function updateHeroProgress() {
  ...
  // Apply to DOM
  const bodyEl = document.querySelector('.body-axis');
  const breathEl = document.querySelector('.breath-circle');
  const attentionEl = document.querySelector('.attention-triangle');
  const spaceEl = document.querySelector('.space-rays');

  if (bodyEl) bodyEl.classList.toggle('active', axesStatus['Body']);
  if (breathEl) breathEl.classList.toggle('active', axesStatus['Breath']);
  if (attentionEl) attentionEl.classList.toggle('active', axesStatus['Attention']);
  if (spaceEl) spaceEl.classList.toggle('active', axesStatus['Space']);
}
...
function launchPractice(file) {
  ...
  // Hide parent FAB when practice starts
...
function openLearningToRide() {
  const file = LANG === 'el' ? 'learning-to-ride-greek.html' : 'learning-to-ride.html';
  launchPractice(file);
}
...
function showToast(msg, duration = 3000) {
  ...
  if (!toast) {
    ...
    toast.id = 'app-toast';
    toast.style.cssText = 'position:fixed;bottom:100px;left:50%;transform:translateX(-50%);background:rgba(20,40,45,0.95);color:#fff;padding:12px 20px;border-radius:12px;font-size:13px;z-index:1000;box-shadow:0 8px 30px rgba(0,0,0,0.5);border:1px solid rgba(168,213,220,0.2);pointer-events:none;text-align:center;max-width:85%;transition:all 0.4s ease;opacity:0';
...
function updateHeroBtnUI() {
  const audioBtn = document.getElementById('hero-btn-audio');
  const voiceBtn = document.getElementById('hero-btn-voice');
  const audioActive = (typeof breathAudioOn !== 'undefined' && breathAudioOn);
  const voiceActive = (typeof bVoiceEnabled !== 'undefined' && bVoiceEnabled);
