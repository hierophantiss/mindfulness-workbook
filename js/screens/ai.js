...
`;
  ...
  try {
    if (typeof generateAIResponse === 'function') {
      // Pass the actual history array now that generateAIResponse supports it
      return await generateAIResponse(aiChatHistory, systemInstruction);
    } else {
      // Fallback if the module hasn't loaded yet
      throw new Error("AI Service not yet initialized. Please wait a moment.");
    }
  } catch (e) {
...
window.buildAIScreen = buildAIScreen;
