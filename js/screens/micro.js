...
// Global state (was previously in js/practice.js which is no longer loaded)
var microCat = (typeof microCat !== 'undefined') ? microCat : 'all';
var microIdx = (typeof microIdx !== 'undefined') ? microIdx : 0;
var microTimer = null;
  ...
  microTimer = setInterval(() => {
    ...
    if (remaining <= 0) {
      ...
      if (el) {
        ...
        setTimeout(() => el.classList.remove('animate-success'), 500);
      }
...
window.buildMicroScreen = buildMicroScreen;
window.toggleMicroScience = toggleMicroScience;
window.startMicroTimer = startMicroTimer;
window.microCat = microCat;
window.microIdx = microIdx;
