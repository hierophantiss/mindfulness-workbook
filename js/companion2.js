...
var COMP_KEY = 'mindful_companion_v5';

function c2SaveCompanionData(data) {
    try { 
        localStorage.setItem(COMP_KEY, JSON.stringify(data));
        // Also update local reference if in different tab/context
...
function c2EnsureState() {
    if (typeof window.companionData === 'undefined' || !window.companionData.dailyLogs) {
        window.companionData = JSON.parse(localStorage.getItem(COMP_KEY)) || {};
    }
...
function c2Init() {
    console.log('Companion 2.0 initializing');
    ...
    sheet.innerHTML = `
        ...
        <div class="c2-sheet-body-wrapper">
            ...
            <div class="c2-sheet-footer">
                <button class="c2-footer-nav-btn" onclick="window.c2GoHome()" aria-label="${lang === 'el' ? 'Αρχική' : 'Home'}">
                    <span class="c2-nav-icon">🏠</span>
                    <span class="c2-nav-label">${lang === 'el' ? 'Αρχική' : 'Home'}</span>
                </button>
                <button class="c2-footer-nav-btn" onclick="window.c2ToggleMenu()" aria-label="${lang === 'el' ? 'Μενού' : 'Menu'}">
                    <span class="c2-nav-icon">☰</span>
                    <span class="c2-nav-label">${lang === 'el' ? 'Μενού' : 'Menu'}</span>
                </button>
                <button class="c2-footer-nav-btn" onclick="window.c2AnalyzeDay()" id="c2-analyze-btn" title="${lang === 'el' ? 'Ανάλυση Ημέρας' : 'Analyze Day'}">
                    <span class="c2-nav-icon">✨</span>
                    <span class="c2-nav-label">${lang === 'el' ? 'Ανάλυση' : 'Analyze'}</span>
                </button>
            </div>
        </div>`;
...
function c2ShowMainMenu() {
    ...
    // DAILY AI PLAN
    if (d.activeDailyPlan) {
        html += `<div class="c2-ai-plan-card" style="margin:0 0 16px 0; padding:14px; background:linear-gradient(135deg, rgba(232, 112, 74, 0.1), rgba(152, 96, 168, 0.1)); border-radius:16px; border:1px solid rgba(152, 96, 168, 0.2); position:relative;">
            <div style="font-size:11px; font-weight:700; color:var(--terra, #E24B4A); margin-bottom:6px; letter-spacing:0.05em; text-transform:uppercase;">✨ ${lang === 'el' ? 'Προσωπική Πρόταση' : 'Personal Suggestion'}</div>
            <div style="font-size:14px; line-height:1.5; color:var(--text); margin-bottom:10px;">${d.activeDailyPlan.msg}</div>
            ${d.activeDailyPlan.action ? `
                <button class="c2-action-btn" style="width:100%; font-size:13px; padding:10px;" onclick="window.c2HideSheet(); ${d.activeDailyPlan.action.run}">
                    ${d.activeDailyPlan.action.label}
                </button>
            ` : ''}
        </div>`;
    }
...
function c2RenderSmartHub(step, lang) {
    ...
    // CONTEXT B: Χρήστης είναι στην αρχική ή αλλού
        ...
        // AI Analysis trigger
        html += `<button class="c2-option-btn" id="ai-analysis-btn" onclick="window.c2RunDailyAnalysis()">✨ ${lang === 'el' ? 'Ανάλυση ημέρας & Πλάνο' : 'Day Analysis & Plan'}</button>`;
...
function c2HandleMoodClick(moodKey) {
    ...
    if (typeof logCompanionActivity === 'function') {
        logCompanionActivity('mood_report', { mood: moodKey });
    }
...
// 10. AI ANALYSIS & PLANNING
async function c2AnalyzeDay() {
    const lang = (typeof LANG !== 'undefined') ? LANG : 'el';
    const d = window.companionData;
    const btn = document.getElementById('c2-analyze-btn');
    if (btn) btn.style.opacity = '0.5';

    const dayContext = {
        moods: d.moodHistory.slice(-5),
        logs: d.dailyLogs.slice(-10),
        progress: d.programProgress,
        chapter: d.lastChapter
    };

    const sysPrompt = lang === 'el' ? 
        `Είσαι ένας ειδικός βοηθός mindfulness για νευροδιαφορετικά άτομα. 
         Ανάλυσε τα δεδομένα της ημέρας του χρήστη και πρότεινε ΜΙΑ συγκεκριμένη άσκηση ή στόχο για ΑΥΡΙΟ.
         Η απάντηση πρέπει να είναι ΣΥΝΤΟΜΗ (1-2 προτάσεις).
         Επίσης, επέλεξε μια κατάλληλη δράση (action) από τις εξής: 'showScreen("breath")', 'window.c2StartFlow("moodCheck")', 'window.c2ShowFaqList()'.`
        : `You are a mindfulness assistant for neurodivergent individuals.
         Analyze the user's daily data and suggest ONE specific exercise or goal for TOMORROW.
         Keep it SHORT (1-2 sentences).
         Also, pick an action from: 'showScreen("breath")', 'window.c2StartFlow("moodCheck")', 'window.c2ShowFaqList()'.`;

    try {
        if (typeof generateAIResponse === 'function') {
            const aiText = await generateAIResponse(JSON.stringify(dayContext), sysPrompt);
            if (aiText) {
                d.activeDailyPlan = {
                    msg: aiText,
                    date: new Date().toLocaleDateString(),
                    action: { label: (lang === 'el' ? 'Έναρξη' : 'Start'), run: 'showScreen("breath")' } // Simplification for now
                };
                c2SaveCompanionData(d);
                c2ShowMainMenu();
                if (typeof showToast === 'function') showToast(lang === 'el' ? 'Το πρόγραμμα για αύριο ετοιμάστηκε!' : 'Tomorrow\'s plan is ready!');
            }
        } else {
...
window.c2AnalyzeDay = c2AnalyzeDay;
window.c2SaveCompanionData = c2SaveCompanionData;
window.c2Init = c2Init;
window.c2GoHome = function() {
    window.c2HideSheet();
    let handled = false;
    try {
        if (window.parent && typeof window.parent.closePractice === 'function') {
            window.parent.closePractice();
            handled = true;
        }
    } catch (e) {}
    if (!handled && typeof showScreen === 'function') showScreen('home');
};

window.c2ToggleMenu = function() {
    window.c2HideSheet();
    let handled = false;
    try {
        ...
        // Handle cross-origin SecurityError
        try {
            if (window.parent) {
                window.parent.postMessage({ type: 'toggleMenu' }, '*');
                handled = true;
            }
        } catch (postMsgErr) {}
    }
    if (!handled && typeof toggleMenu === 'function') toggleMenu();
};
...
// 9. AI Daily Analysis Implementation
async function c2RunDailyAnalysis() {
    var lang = (typeof LANG !== 'undefined') ? LANG : 'el';
    var d = window.companionData;
    var btn = document.getElementById('ai-analysis-btn');
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = `⌛ ${lang === 'el' ? 'Αναλύω...' : 'Analyzing...'}`;
    }

    // Filter logs for last 24h
    var now = new Date().getTime();
    var dayLogs = d.dailyLogs.filter(l => (now - new Date(l.t).getTime()) < 86400000);
    
    var systemInstruction = `You are the ∞ Mindfulness Companion. 
Analyze the user's recent activity logs. 
Create a supportive, non-judgmental summary of their day and suggest ONE next step (Chapter or Exercise) for "tomorrow".
Focus on grounding if mood was low, or curiosity if they were active.
Be very brief (max 2 sentences for the message). 

OUTPUT MUST BE VALID JSON:
{
  "msg": "supportive message text",
  "action": { "label": "Button Label", "run": "JS code to run on click" }
}

JS Code options:
- "openChapter(N)" (N is 1-10)
- "showScreen('breath')"
- "showScreen('practice')"
- "microCat='body';showScreen('micro')"
- "launchPractice('openawareness.html')"

Language: ${lang === 'el' ? 'Greek' : 'English'}`;

    var userPrompt = `DAILY LOGS: ${JSON.stringify(dayLogs)}
    Current State: Chapter ${d.lastChapter}, Program Week ${d.programProgress.week}.`;

    try {
        if (typeof generateAIResponse === 'function') {
            const aiResponseText = await generateAIResponse(userPrompt, systemInstruction);
            
            // Try to parse the JSON output from AI
            var planData;
            try {
                // AI might wrap in markdown ```json ... ```
                var clean = aiResponseText.replace(/```json|```/gi, '').trim();
                planData = JSON.parse(clean);
            } catch(e) {
                // Fallback if AI fails to return perfect JSON
                planData = { 
                    msg: aiResponseText, 
                    action: { label: lang === 'el' ? 'Συνέχεια' : 'Continue', run: 'window.c2ShowMainMenu()' } 
                };
            }
            ...
            d.activeDailyPlan = {
                msg: planData.msg,
                date: new Date().toLocaleDateString(),
                action: planData.action || { label: lang === 'el' ? 'Έναρξη' : 'Start', run: 'showScreen("breath")' }
            };
            c2SaveCompanionData(d);
            c2ShowMainMenu();
            if (typeof showToast === 'function') showToast(lang === 'el' ? 'Η ανάλυση ολοκληρώθηκε!' : 'Analysis complete!');
        ...
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = `⚠️ ${lang === 'el' ? 'Πρόβλημα σύνδεσης' : 'Connection problem'}`;
        }
    } finally {
        if (btn && btn.innerHTML.includes('...')) {
             btn.disabled = false;
             btn.innerHTML = `✨ ${lang === 'el' ? 'Ανάλυση Ημέρας' : 'Analyze Day'}`;
        }
    }
}

// 10. Sync from other tabs/scripts
window.addEventListener('companionDataUpdated', function(e) {
    window.companionData = e.detail;
});

window.c2RunDailyAnalysis = c2RunDailyAnalysis;
