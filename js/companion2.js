...
function c2EnsureState() {
    if (typeof window.companionData === 'undefined') {
        window.companionData = JSON.parse(localStorage.getItem('companion_v1')) || {};
    }
...
function c2ShowMainMenu() {
    ...
    // ─── AI PROGRESS SYNTHESIS (NEW) ───
    if (d.lastAiSynthesis) {
        var s = d.lastAiSynthesis;
        html += `
        <div class="c2-ai-synthesis-card" style="margin: 0 0 16px 0; padding:16px; background: linear-gradient(135deg, rgba(122,158,126,0.15) 0%, rgba(42,93,94,0.1) 100%); border-radius:16px; border:1px solid rgba(122,158,126,0.3); animation: fadeSlideUp 0.6s ease;">
            <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px;">
                <span style="font-size:24px;">${s.icon || '♾️'}</span>
                <div style="font-weight:700; color:var(--teal); font-size:14px; letter-spacing:0.3px;">${lang === 'el' ? 'Η ΠΟΡΕΙΑ ΣΟΥ' : 'YOUR JOURNEY'}</div>
            </div>
            <div style="font-size:14px; line-height:1.5; color:var(--text); margin-bottom:10px;">${s.summary}</div>
            <div style="font-size:12px; font-style:italic; opacity:0.8; margin-bottom:12px; border-left:2px solid var(--teal); padding-left:10px;">${s.insight}</div>
            <div style="background:rgba(255,255,255,0.3); padding:10px; border-radius:8px; border:1px dashed rgba(0,0,0,0.1);">
                <div style="font-size:11px; font-weight:600; opacity:0.6; margin-bottom:4px; text-transform:uppercase;">${lang === 'el' ? 'ΕΠΟΜΕΝΟ ΒΗΜΑ' : 'NEXT STEP'}</div>
                <div style="font-size:13px; font-weight:600;">${s.nextStep}</div>
            </div>
        </div>`;
    }
