...
var c2IsDragging = false;
var c2ChapterScrollTracker = null;
var c2ChapterTimeStart = null;
...
function c2EnsureState() {
    if (typeof window.companionData === 'undefined' || !window.companionData) {
        var saved = localStorage.getItem('companion_v1');
        window.companionData = saved ? JSON.parse(saved) : {};
    }
    var d = window.companionData;
    if (!d.chapterProgress) d.chapterProgress = {};
    if (!d.programProgress) d.programProgress = { week: 0, day: 0, lastVisit: null };
    ...
    c2SaveCompanionData(d);
}

function c2SaveCompanionData(data) {
    try { 
        window.companionData = data;
        localStorage.setItem('companion_v1', JSON.stringify(data)); 
    } catch(e) {}
}

// 2. Δημιουργία του UI (Sheet & FAB)
function c2Init() {
    ...
    sheet.innerHTML = `
        ...
        <div class="c2-sheet-body-wrapper">
            ...
            <div class="c2-sheet-footer">
                <button class="c2-footer-nav-btn" onclick="window.c2HideSheet(); showScreen('home')" aria-label="${lang === 'el' ? 'Αρχική' : 'Home'}">
                    <span class="c2-nav-icon">🏠</span>
                    <span class="c2-nav-label">${lang === 'el' ? 'Αρχική' : 'Home'}</span>
                </button>
                <button class="c2-footer-nav-btn" onclick="window.c2HideSheet(); toggleMenu()" aria-label="${lang === 'el' ? 'Μενού' : 'Menu'}">
                    <span class="c2-nav-icon">☰</span>
                    <span class="c2-nav-label">${lang === 'el' ? 'Μενού' : 'Menu'}</span>
                </button>
            </div>
        </div>`;
    document.body.appendChild(sheet);

    // Create FAB (Floating Infinity Button)
    if (!document.getElementById('companion-fab')) {
        var fab = document.createElement('div');
        fab.id = 'companion-fab';
        fab.className = 'fab-floating';
        fab.innerHTML = `<svg width="36" height="20" viewBox="0 0 120 66" fill="none" xmlns="http://www.w3.org/2000/svg">
            <defs><linearGradient id="inf-grad" x1="0" y1="33" x2="120" y2="33" gradientUnits="userSpaceOnUse">
            <stop offset="0%" stop-color="#E8704A"/><stop offset="100%" stop-color="#9860A8"/>
            </linearGradient></defs>
            <path d="M60 33 C60 16, 45 4, 30 4 C15 4, 2 16, 2 33 C2 50, 15 62, 30 62 C45 62, 60 50, 60 33 C60 16, 75 4, 90 4 C105 4, 118 16, 118 33 C118 50, 105 62, 90 62 C75 62, 60 50, 60 33Z" stroke="url(#inf-grad)" stroke-width="8" stroke-linecap="round" fill="none"/>
        </svg>`;
        
        c2UpdateFabPosition(fab);
        fab.addEventListener('click', function(e) {
            if (c2IsDragging) return;
            e.stopPropagation();
    ...
    window.showScreen = function(id) {
        ...
        window.companionTrackScreen(id);
    };

    var origOpenChapter = window.openChapter;
    window.openChapter = function(num) {
        if (origOpenChapter) origOpenChapter(num);
        window.companionStartChapterTracking(num);
    };

    // Tracking
    window.addEventListener('resize', () => c2UpdateFabPosition(document.getElementById('companion-fab')));
}

// ── FAB POSITIONING & DRAG ──
function c2UpdateFabPosition(fab) {
    if (!fab) return;
    var d = window.companionData;
    var screenW = window.innerWidth;
    var maxW = screenW >= 900 ? 860 : (screenW >= 600 ? 720 : screenW);
    var leftEdge = (screenW - maxW) / 2;
    var rightEdge = leftEdge + maxW;

    if (d && d.fabPos && typeof d.fabPos.x === 'number') {
        var x = Math.max(leftEdge + 8, Math.min(rightEdge - 60, d.fabPos.x));
        var y = Math.max(70, Math.min(window.innerHeight - 110, d.fabPos.y));
        fab.style.left = x + 'px';
        fab.style.top = y + 'px';
    } else {
        fab.style.left = (rightEdge - 68) + 'px';
        fab.style.top = (window.innerHeight / 2 - 26) + 'px';
    }
}

function c2MakeDraggable(el) {
    var startX, startY;
    function onMove(e) {
        c2IsDragging = true;
        ...
        var x = clientX - startX, y = clientY - startY;
        el.style.left = x + 'px';
        el.style.top = y + 'px';
    }
    function onEnd() {
        ...
        if (c2IsDragging) {
            ...
            window.companionData.fabPos = { x: parseInt(el.style.left), y: parseInt(el.style.top) };
            c2SaveCompanionData(window.companionData);
            setTimeout(() => { c2IsDragging = false; }, 50);
        }
    }
    el.addEventListener('mousedown', e => {
        startX = e.clientX - el.offsetLeft; startY = e.clientY - el.offsetTop;
        document.addEventListener('mousemove', onMove); document.addEventListener('mouseup', onEnd);
    });
    el.addEventListener('touchstart', e => {
        startX = e.touches[0].clientX - el.offsetLeft; startY = e.touches[0].clientY - el.offsetTop;
        document.addEventListener('touchmove', onMove); document.addEventListener('touchend', onEnd);
    }, {passive:true});
}

// ── PROGRESS TRACKING (Legacy compat) ──
window.companionTrackScreen = function(screenId) {
    c2EnsureState();
    var d = window.companionData;
    d.lastScreen = screenId;
    d.lastSeen = new Date().toISOString();
    if (!d.visits) d.visits = [];
    d.visits.push({ date: d.lastSeen, screen: screenId });
    if (d.visits.length > 50) d.visits = d.visits.slice(-50);
    c2SaveCompanionData(d);
    if (screenId !== 'chapter') window.companionStopChapterTracking();
};

window.companionStartChapterTracking = function(num) {
    window.companionStopChapterTracking();
    c2ChapterTimeStart = Date.now();
    var d = window.companionData;
    d.lastChapter = num;
    if (!d.chapterProgress[num]) {
        d.chapterProgress[num] = { scrollPct: 0, timeSpent: 0, lastVisit: new Date().toISOString(), completed: false, visits: 0 };
    }
    d.chapterProgress[num].visits++;
    d.chapterProgress[num].lastVisit = new Date().toISOString();
    c2SaveCompanionData(d);

    setTimeout(() => {
        var sa = document.getElementById('screen-chapter') ? document.getElementById('screen-chapter').querySelector('.scroll-area') : null;
        if (sa) {
            c2ChapterScrollTracker = function() {
                var pct = sa.scrollTop / (sa.scrollHeight - sa.clientHeight);
                var capped = Math.min(1, Math.max(0, pct));
                if (d.chapterProgress[num] && capped > d.chapterProgress[num].scrollPct) {
                    d.chapterProgress[num].scrollPct = Math.round(capped * 100) / 100;
                    if (capped > 0.85) d.chapterProgress[num].completed = true;
                    c2SaveCompanionData(d);
                }
            };
            sa.addEventListener('scroll', c2ChapterScrollTracker, { passive: true });
        }
    }, 300);
};

window.companionStopChapterTracking = function() {
    if (c2ChapterTimeStart && window.companionData.lastChapter) {
        var elapsed = Math.round((Date.now() - c2ChapterTimeStart) / 1000);
        var d = window.companionData;
        if (d.chapterProgress[d.lastChapter]) {
            d.chapterProgress[d.lastChapter].timeSpent += elapsed;
            c2SaveCompanionData(d);
        }
    }
    c2ChapterTimeStart = null;
    var sa = document.getElementById('screen-chapter') ? document.getElementById('screen-chapter').querySelector('.scroll-area') : null;
    if (sa && c2ChapterScrollTracker) sa.removeEventListener('scroll', c2ChapterScrollTracker);
    c2ChapterScrollTracker = null;
};

// ── EXPORT / IMPORT (Legacy compat) ──
window.c2ExportData = function() {
    var data = localStorage.getItem('companion_v1');
    if (!data) return;
    var blob = new Blob([data], { type: 'application/json' });
    var url = URL.createObjectURL(blob);
    var a = document.createElement('a');
    a.href = url;
    a.download = 'mindfulness_backup.json';
    a.click();
};

window.c2ImportData = function(file) {
...
// Legacy Bridge for 8-week program
window.syncWithCompanion = function() {
    c2EnsureState();
    var d = window.companionData;
    // The HTML file usually sets W and DAY
    if (typeof W !== 'undefined' && typeof DAY !== 'undefined') {
        if (W > d.programProgress.week || (W === d.programProgress.week && DAY >= d.programProgress.day)) {
            d.programProgress.week = W;
            d.programProgress.day = DAY;
            if (typeof D !== 'undefined' && D[W] && D[W].days[DAY-1]) {
                d.programProgress.title = D[W].days[DAY-1].title;
            }
        }
    }
    d.programProgress.lastVisit = new Date().toISOString();
    c2SaveCompanionData(d);
};

// Self Initialize
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', c2Init);
} else {
    c2Init();
}
...
function c2ShowSheet() {
    ...
    if (!sheet) {
        c2Init();
        sheet = document.getElementById('c2-sheet');
    }
    if (!sheet) return;
    ...
    // Defer heavy rendering to next frame to allow the animation to start immediately
    requestAnimationFrame(() => {
        c2EnsureState();
        if (!c2CurrentFlow) {
            window.c2ShowMainMenu();
        } else if (c2CurrentStep) {
            window.c2RunStep(c2CurrentStep.id);
        }
...
function c2RefreshHeader() {
    if (c2SheetVisible && !c2CurrentFlow) {
        c2ShowMainMenu();
    }
}
...
function c2ShowMainMenu() {
    var lang = (typeof LANG !== 'undefined') ? LANG : 'el';
    var d = window.companionData;
    var isProgramPage = window.location.pathname.includes('learning-to-ride');
    ...
    // ADD PROGRAM PROGRESS DIRECTLY IF ON PROGRAM PAGE
    if (isProgramPage && d.programProgress && d.programProgress.week > 0) {
        var progTitle = d.programProgress.title || "...";
        html += `<div class="c2-context-box" style="margin: 0 0 14px 0; padding:12px; background:rgba(42,93,94,0.1); border-radius:12px; font-size:13px; border:1px solid rgba(42,93,94,0.2)">
            <div style="font-weight:bold; margin-bottom:4px; color:var(--teal); display:flex; align-items:center; gap:6px;">
                <span>📅</span> ${lang === 'el' ? 'Η Πρόοδός σου:' : 'Your Progress:'}
            </div>
            <div style="font-size:14px; font-weight:600; margin-bottom:4px;">${lang === 'el' ? 'Εβδομάδα' : 'Week'} ${d.programProgress.week}, ${lang === 'el' ? 'Ημέρα' : 'Day'} ${d.programProgress.day}</div>
            <div style="opacity:0.8; font-size:12px; margin-bottom:8px;">${progTitle}</div>
            <div style="font-style:italic; opacity:0.7; font-size:11px;">${lang === 'el' ? 'Συνέχισε την εξαιρετική προσπάθεια.' : 'Keep up the great work.'}</div>
        </div>`;
    }

    html += `<div class="c2-menu">
            ${c2MenuBtn('💭', lang === 'el' ? 'Πώς νιώθεις;' : 'How do you feel?', "window.c2StartFlow('moodCheck')")}
            ${c2MenuBtn('🧭', lang === 'el' ? 'Έξυπνη Καθοδήγηση' : 'Smart Guidance', "window.c2StartFlow('smartHub')")}
        </div>`;
    c2SetContent(html);
}

function c2MenuBtn(icon, title, action) {
    return `
    <button class="c2-menu-btn" onclick="${action}">
...
function c2RunStep(stepId) {
    ...
    // Track history for back button support
    if (c2CurrentStep && c2CurrentStep.id !== stepId) {
        if (c2FlowHistory.indexOf(c2CurrentStep.id) === -1) {
            c2FlowHistory.push(c2CurrentStep.id);
        }
    }
    ...
    } else if (step.type === 'history_view') {
        window.c2RenderHistory(step, lang);
...
function c2RenderHistory(step, lang) {
    var d = window.companionData;
    var html = `<div class="c2-back-row"><button class="c2-back" onclick="window.c2ShowMainMenu()">←</button><span class="c2-back-title">${lang === 'el' ? 'Ιστορικό & Σημειώσεις' : 'History & Notes'}</span></div>`;
    ...
    // Chapter notes
    if (d.journalHistory) {
        Object.keys(d.journalHistory).forEach(ch => {
            allNotes.push({
                type: 'chapter',
                source: lang === 'el' ? `Κεφάλαιο ${ch}` : `Chapter ${ch}`,
                text: d.journalHistory[ch],
                date: d.lastSeen || new Date().toISOString(), // Fallback
                timestamp: d.journalHistory[ch].timestamp || 0
            });
        });
    }
    ...
    // Program notes
    if (d.programJournal) {
        Object.keys(d.programJournal).forEach(key => {
            var match = key.match(/w(\d+)d(\d+)/);
            var label = match ? (lang === 'el' ? `W${match[1]}D${match[2]}` : `W${match[1]}D${match[2]}`) : key;
            allNotes.push({
                type: 'program',
                source: `${lang === 'el' ? 'Πρόγραμμα' : 'Program'} — ${label}`,
                text: d.programJournal[key].text,
                date: d.programJournal[key].date,
                timestamp: d.programJournal[key].timestamp || 0
            });
        });
    }
    ...
    if (allNotes.length === 0) {
        html += `<div style="text-align:center; opacity:0.6; padding:40px 0;">${lang === 'el' ? 'Δεν υπάρχουν σημειώσεις ακόμα.' : 'No notes yet.'}</div>`;
    } else {
        allNotes.forEach(n => {
            html += `<div class="c2-history-item" style="margin-bottom:12px; border-bottom:1px solid var(--border); padding-bottom:12px;">
                <div style="font-size:11px; opacity:0.8; margin-bottom:4px; color:var(--teal); font-weight:bold;">${n.source} · ${n.date}</div>
                <div style="white-space: pre-wrap; font-size:14px; color:var(--text); line-height:1.6;">${n.text}</div>
            </div>`;
        });
    }
...
function c2GoBack() {
    if (c2FlowHistory.length > 0) {
        var prevStepId = c2FlowHistory.pop();
        window.c2RunStep(prevStepId);
    } else {
        window.c2ShowMainMenu();
    }
}
...
function c2RenderSmartHub(step, lang) {
    ...
    // Check if we are on the 8-week program page
    var isProgramPage = window.location.pathname.includes('learning-to-ride');

    var html = `<div class="c2-back-row"><button class="c2-back" onclick="window.c2GoBack()" aria-label="${lang === 'el' ? 'Πίσω' : 'Back'}">←</button><span class="c2-back-title" role="heading" aria-level="2">${lang === 'el' ? 'Τι να κάνω;' : 'What to do?'}</span></div>`;
    
    // Memory/First time logic
    if (!d.lastChapter && (!d.programProgress || d.programProgress.week === 0)) {
        html += `<div class="c2-welcome-wisdom" style="margin-bottom:12px; font-size:12px;">
            ${lang === 'el' ? '<b>Πρώτη φορά;</b> Αυτή η εφαρμογή είναι ένας οδηγός Mindfulness. Ξεκίνα από το Κεφάλαιο 1 ή το Πρόγραμμα 8 Εβδομάδων.' : '<b>First time?</b> This app is a Mindfulness guide. Start from Chapter 1 or the 8-Week Program.'}
        </div>`;
    } else {
        // Context-aware guidance
        if (isProgramPage && d.programProgress && d.programProgress.week > 0) {
             var progTitle = d.programProgress.title || "...";
             html += `<div class="c2-context-box" style="margin-bottom:12px; padding:12px; background:rgba(42,93,94,0.1); border-radius:8px; font-size:13px; border:1px solid var(--border);">
                <div style="font-weight:bold; margin-bottom:2px;">${lang === 'el' ? 'Πρόοδος Προγράμματος:' : 'Program Progress:'}</div>
                <div style="font-size:14px; margin-bottom:4px;">Week ${d.programProgress.week}, Day ${d.programProgress.day} <span style="opacity:0.7; font-size:12px;">— ${progTitle}</span></div>
                <div style="font-style:italic; opacity:0.8;">${lang === 'el' ? 'Κάθε βήμα μετράει. Είσαι στο σωστό δρόμο.' : 'Every step counts. You are on the right track.'}</div>
            </div>`;
        } else {
    ...
    // CONTEXT A: Χρήστης διαβάζει κεφάλαιο ή είναι στο πρόγραμμα
    if (activeScreen === 'screen-chapter' || activeScreen === 'screen-chapter' || isProgramPage) {
        ...
        html += `<button class="c2-option-btn" onclick="window.c2StartFlow('chapterExplore')">❓ ${lang === 'el' ? 'Απορίες' : 'Questions?'}</button>`;
    } 
    // CONTEXT B: Χρήστης είναι στην αρχική ή αλλού
    else {
        if (d.programProgress && d.programProgress.week > 0) {
            var progUrl = lang === 'el' ? 'learning-to-ride-greek.html' : 'learning-to-ride-english.html';
            html += `<button class="c2-option-btn" onclick="window.location.href='${progUrl}'">📅 ${lang === 'el' ? 'Συνέχισε το Πρόγραμμα' : 'Continue 8-Week Program'}</button>`;
        }
    ...
    html += `<button class="c2-option-btn" style="background:rgba(42,93,94,0.15); border:1px solid var(--teal);" onclick="window.c2RunApiTurn('User requested explicit AI guidance from Smart Hub.')">♾️ ${lang === 'el' ? 'Ρώτησε την Τεχνητή Νοημοσύνη' : 'Ask the AI Intelligence'}</button>`;
...
function c2RenderGenericSelect(step, lang) {
    var title = step.text ? step.text[lang] : "...";
    var html = `<div class="c2-back-row"><button class="c2-back" onclick="window.c2GoBack()">←</button><span class="c2-back-title">${title}</span></div>`;
    html += '<div class="c2-options">';
...
// 7. Context Hints & Reminders (Muted per user request for no popups)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Daily reminder (Logging only)
    setTimeout(function() { 
        console.log("Daily reminder system active (silent mode)");
    }, 1000);

    /* 
    // Έναρξη welcome flow αν είναι η πρώτη φορά ή επιστροφή (DISABLED per user request to avoid auto popups)
    setTimeout(function() {
        if (!c2SheetVisible) {
