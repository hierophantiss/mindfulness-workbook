...
function initCompanion() {
  ...
  fab.addEventListener('click', function(e) {
    ...
    // Trigger AI Companion Turn
    if (window.c2RunApiTurn) {
      window.c2RunApiTurn('User clicked the companion FAB. Welcome them and provide a short suggestion.');
...
function updateFabPosition(fab) {
  ...
  if (companionData.fabPos && typeof companionData.fabPos.x === 'number') {
    ...
    var x = Math.max(bounds.left + 8, Math.min(bounds.right - 60, companionData.fabPos.x));
    var y = Math.max(70, Math.min(window.innerHeight - 110, companionData.fabPos.y));
    fab.style.left = x + 'px';
    fab.style.top = y + 'px';
  } else {
    // Default position: Middle-Right of the centered frame
    var fabH = 52;
    var xDefault = bounds.right - 68;
